...
leipzig_Bezirke <-
  readOGR(
    dsn = "./shapefiles/Leipzig_Stadtbezirke_UTM33N",
    layer = "sbz",
    use_iconv = TRUE,
    encoding = "UTF-8"
  ) 
leipzig_Bezirke <-
  spTransform(leipzig_Bezirke, CRS("+proj=longlat +datum=WGS84"))

Bezirke_NDVI <-
  read.csv(file = './data/sbz_ndvi.csv',
           row.names = NULL,
           encoding = "UTF-8")
Bezirke_EVI <-
  read.csv(file = './data/sbz_evi.csv',
           row.names = NULL,
           encoding = "UTF-8")
Bezirke_Einkommen <-
  read.csv(
    file = './data/Einkommen_und_Preise_Nettoeinkommen_SBZ.csv',
    sep = ";",
    dec = ",",
    row.names = NULL,
    encoding = "UTF-8"
...
#ortsteile:
leipzig_Ortsteile = readOGR(
  dsn = "./shapefiles/Leipzig_Ortsteile_UTM33N",
  layer = "ot",
  use_iconv = TRUE,
  encoding = "UTF-8"
) 
leipzig_Ortsteile <-
  spTransform(leipzig_Ortsteile, CRS("+proj=longlat +datum=WGS84"))

OT_NDVI <-
  read.csv(file = './data/ot_ndvi.csv',
           row.names = NULL,
           encoding = "UTF-8")
OT_EVI <-
  read.csv(file = './data/ot_evi.csv',
           row.names = NULL,
           encoding = "UTF-8")
OT_Einkommen <-
  read.csv(
    file = './data/Einkommen_und_Preise_Nettoeinkommen_OT.csv',
    sep = ";",
    dec = ",",
    row.names = NULL,
    encoding = "UTF-8"
...
labels <-
  paste(
    "<p>",
    "Bezirk: ",
    Bezirksdaten_gesamt$Name,
    "</p>",
    "<p>",
    "NDVI: ",
    round(Bezirksdaten_gesamt$NDVI, digits = 3) ,
    "</p>",
    "<p>",
    "EVI: ",
    round(Bezirksdaten_gesamt$EVI, digits = 3) ,
    "</p>",
    "<p>",
    "Median income: ",
    round(Bezirksdaten_gesamt$Einkommen, digits = 2) ,
    " Euro. (Rank " ,
    Bezirksdaten_gesamt$X ,
    " of ",
    nrow(Bezirksdaten_gesamt) ,
    ")",
    "</p>",
    sep = ""
  )
...
labels2 <-
  paste(
    "<p>",
    "Ortsteil: ",
    Ortsteildaten_gesamt$Name,
    "</p>",
    "<p>",
    "NDVI: ",
    round(Ortsteildaten_gesamt$NDVI, digits = 3) ,
    "</p>",
    "<p>",
    "EVI: ",
    round(Ortsteildaten_gesamt$EVI, digits = 3) ,
    "</p>",
    "<p>",
    "Median income: ",
    round(Ortsteildaten_gesamt$Einkommen, digits = 2) ,
    " Euro. (Rank " ,
    Ortsteildaten_gesamt$X ,
    " of ",
    nrow(Ortsteildaten_gesamt) ,
    ")",
    "</p>",
...
ui <- fillPage(
  ...
  mainPanel(title = "Green spaces Leipzig",),
  ...
  absolutePanel(
    ...
    bsTooltip(
      "radiusslider",
      "check the box above to calculate NDVI with this radius",
      placement = "bottom",
      trigger = "hover",
      options = NULL
    ),
    bsTooltip(
      "umkreisBox",
      "check this box and click on the map to calculate the NDVI for the clicked area",
      placement = "bottom",
      trigger = "hover",
      options = NULL
    )
  ...
  bsTooltip(
    "legende",
    "the NDVI values range from -1 to 1. Zero means no vegatation, a value close to one indicates the highest possible density of green leaves. Negative values indicate water.",
    placement = "top",
    trigger = "hover",
    options = NULL
  ),
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (input$legende) {
      proxy %>% addLegend(
        ...
        pal = binpal,
        values = Ortsteildaten_gesamt$NDVI,
  ...
  observe({
    if (input$umkreisBox) {
      ...
      showModal(
        modalDialog(
          "Calculating NDVI for latest click and radius. This may take a minute or two...",
          footer = NULL
        )
      )
      point <- data.frame(ID = 1,
                          X = c(click$lng),
                          Y = c(click$lat))
      ...
      point <-
        spTransform(point,
                    CRS("+proj=utm +zone=32 +datum=WGS84 +ellps=WGS84")) %>% data.frame()
      
      circle <-
        spCircle(
          input$radiusslider,
          CRS("+proj=utm +zone=32 +datum=WGS84 +ellps=WGS84"),
          centerPoint = c(x = point[1, 2], y = point[1, 3])
        )
      circle_ndvi <-
        compute_vegetation_index(tif_ndvi, circle$spCircle)
      removeModal()
      
      text <-
        paste("NDVI for selected radius: " , round(circle_ndvi, 3))
